...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<ErrorDTO> handleProfesorException (ProfesorException exception) {

        if(exception.getMessage().contains("no se encuentra en la base de datos")){
            Map<String, String> errorMap = Collections.singletonMap("El profesor no encontrado", exception.getMessage());
